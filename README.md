...
```bash
catalonia-associations sam build --use-container
```

The SAM CLI installs dependencies defined in `catalonia-associations/requirements.txt`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
catalonia-associations sam local invoke ETL --event events/event.json
```
...
```bash
catalonia-associations sam logs -n ETL --stack-name catalonia-associations --tail
```
...
```bash
catalonia-associations pip install pytest pytest-mock --user
catalonia-associations python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name catalonia-associations
```
